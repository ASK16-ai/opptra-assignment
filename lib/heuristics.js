...
export function computeForecast(listing, recommended, bucket, cfg = GUT_RULES) {
  ...
  // Baseline + Buy Box lift, then a small price-elasticity adjustment for
  // moves that don't flip Buy Box status. Recover moves get a ramp-up
  // discount because the algorithm takes 48-72h to fully credit the move.
  const baseNoBB = cfg.BASELINE_DAILY_ORDERS_NO_BB;
  const liftMult = cfg.BUY_BOX_LIFT_MULT;

  const currentOrdersPerDay = currentlyWinning ? baseNoBB * liftMult : baseNoBB;
