...
export const GUT_RULES = {
  ...
  /* RAISE_HEADROOM_PCT / RAISE_HEADROOM_INR — DEPRECATED.
     Originally gated raise recommendations behind a 5% / Rs.50 minimum
     gap, but per category-lead feedback that filtered out small but real
     headroom (e.g. Rs.11 above competitor). The current engine raises
     whenever competitor − UNDERCUT_INR is still above our price. These
     constants are retained for any external code that imports them. */
  RAISE_HEADROOM_PCT: 0,
  RAISE_HEADROOM_INR: 0,
  ...
  /* RECOVERY_DECAY_DAYS — "longer staleness = deeper algorithm decay"
     ...
     confidence and emit a Recovery-decay pattern flag. */
...
export function computeRecommendation(listing, ctx, cfg = GUT_RULES) {
  ...
  if (competitorPrice < recommendedFloor) {
    ...
    // Any positive headroom is actionable. The category lead doesn't want
    // to leave money on the table just because the gap is small — even a
    // Rs.1–10 raise to match-and-just-undercut the competitor is worth
    // recommending. The old "≥5% / ≥Rs.50" threshold over-filtered.
    //
    // We only fall back to HOLD when the proposed raise wouldn't actually
    // lift our price (candidate at or below current) or would cross the
    // floor cushion (rare for the raise path but defended for safety).
    const candidate = competitorPrice - cfg.UNDERCUT_INR;
    if (candidate > ourPrice && candidate >= recommendedFloor) {
      bucket = "raise";
...
2. "reasoning_steps" — an ordered array of 5–6 objects walking through HOW you arrived at the recommendation. Each object:
